...
$SAG_HOME/common/db/bin/dbConfigurator.sh -a create -d "$MWS_DB_TYPE" -pr MywebMethodsServer -l "$MWS_DB_URL" -u "$MWS_DB_USERNAME" -p "$MWS_DB_PASSWORD" -n "$MWS_DB_NAME"
$SAG_HOME/common/db/bin/dbConfigurator.sh -a migrate -d "$MWS_DB_TYPE" -pr MywebMethodsServer -l "$MWS_DB_URL" -u "$MWS_DB_USERNAME" -p "$MWS_DB_PASSWORD" -n "$MWS_DB_NAME"

# Update config
$SAG_HOME/MWS/bin/mws.sh update -Ddb.type="$MWS_DB_TYPE" -Ddb.url="$MWS_DB_URL" -Ddb.username="$MWS_DB_USERNAME" -Ddb.password="$MWS_DB_PASSWORD"
